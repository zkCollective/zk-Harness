...
func (w *uint64api) add(i1, i2 xuint64, in ...xuint64) xuint64 {
	var v []frontend.Variable
	for _, i := range in {
		v = append(v, w.fromUint64(i))
	}
	sum := w.api.Add(w.fromUint64(i1), w.fromUint64(i2), v...)

	b := bits.ToBinary(w.api, sum, bits.WithNbDigits(68))
	var res xuint64
	copy(res[:], b)
